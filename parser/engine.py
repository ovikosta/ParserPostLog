...
class EngineParser:
    ...
    def parse(self):
        ...
        self.write_in_file(count_total, count_ok, count_fail)

    def write_in_file(self, total, ok, fail):
        with open('./result', 'w') as result_file:
            for item in total.most_common():
                result_file.write('E-MAIL: %s; TOTAL: %s; OK: %s; FAIL: %s\n' % (item[0], item[1], ok[item[0]], fail[item[0]]))

if __name__ == '__main__':
    EngineParser(sys.argv).parse()
